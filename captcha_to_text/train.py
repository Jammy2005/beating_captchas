...
def decode_ground_truth(int_labels, vocab, pad_token=None):
    """Converts a numeric label array into a string using `vocab`."""
    chars = []
    for idx in int_labels:
        # Skip blank/padding if needed
        if pad_token is not None and idx == pad_token:
            continue
        if idx < len(vocab):
